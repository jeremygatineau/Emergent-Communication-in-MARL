...
class AriaRE(nn.Module):
    def __init__(self,opt_params):
        super(AriaRE, self).__init__()
        self.batch_size = opt_params["batch_size"]
        self.gamma = opt_params["gamma"]
        self.hiddenDim = 10
        self.memory_size = 10
        self.obs_Mod = lin_Mod([2, 5])
    ...
    def select_action(self, obs, msg):
        ...
        action, message, hid_state = self.forward(obs_t.float(), msg_t.float(), self.memories[self.batch_counter])
